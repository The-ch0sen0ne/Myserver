...
void TcpConnection::readtobuffer()
{

    
    int saveerrno;
    int n = readbuffer_.readfromfd(fd_,&saveerrno);
    if(n == 0)
    {
    ...
    {
        std::cerr << "errno fd = "<< fd_ <<"read error errno = " << saveerrno << std::endl;
    ...
    else
    {
        ...
        //向桶中添加词条
        std::shared_ptr<Entry> en = entry_.lock();
        if(en)
        {
            epoller_->addtocircularbuffer(en);
        }
        handleread();
...
void TcpConnection::destoryconnection()
{
    ...
    //先取消关注事件
    std::cout << "destory connection " << std::endl;
    
    epoller_->removeevent(fd_);
    // std::cout << "before list size = " << epoller_->listsize();    
    epoller_->removefromlist(fd_);
        // std::cout << "after list size = " << epoller_->listsize();    
...
void TcpConnection::handleread()
{
    ...
    {
        const char* temp = "HTTP/1.1 400 Bad Request\r\n\r\n";
        int n = write(fd_,temp,strlen(temp));
        if(n < 0)
        {
...
void TcpConnection::request()
{
    ...
    const std::string& connection = context_.request().getHeader("Connection");
    bool close = connection == "close" ||
    (context_.request().getversion() == HttpRequest::kHttp10 && connection != "Keep-Alive");
    HttpResponse response(close);
    if(context_.request().path() == "/")
    {
        response.setStatusCode(HttpResponse::k200Ok);
        response.setStatusMessage("OK");
        response.setContentType("text/html");
        response.addHeader("Server", "Chosen");
     
        response.setBody("<html><head><title>This is title</title></head><body><h1>Hello</h1>Now is 20220806 09:01:47.910969</body></html>");
    }
    else if(context_.request().path() == "/hello")
    {
        response.setStatusCode(HttpResponse::k200Ok);
        response.setStatusMessage("OK");
    ...
    else if(context_.request().path() == "/favicon.ico")
    {
        response.setStatusCode(HttpResponse::k200Ok);
        response.setStatusMessage("OK");
    ...
    ConnectionBuffer temp;
    response.appendToBuffer(&temp);
    ...
    if(close)
    {
        // std::cout << "not keep-alive need to close" << std::endl;
        destoryconnection();
    }
...
void TcpConnection::send(ConnectionBuffer& buf)
{
    int rest = buf.readable();
    if(writebuffer_.readable() == 0)
    {
        int n = write(fd_,buf.peek(),buf.readable());
        ...
        //debug
        buf.retrive(n);
        // std::cout << "last " << buf.readable() << "bytes" << std::endl;
    }

    
    if(rest != 0)
    {
        //判断写缓冲区当前是否为空，为空则要重写关注写事件
        int istrue = writebuffer_.readable();
        writebuffer_.append(buf.peek(),buf.readable());       
        if(istrue == 0)
        {
            epoller_->addwrite(fd_);
        }
    }
} 
...
{
    int n = write(fd_,writebuffer_.peek(),writebuffer_.readable());
    if(n > 0)
    {
        writebuffer_.retrive(n);
        if(writebuffer_.readable())
        {
            epoller_->cancelwrite(fd_);
        }
    }
}
